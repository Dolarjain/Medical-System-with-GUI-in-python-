...
def send_otp():
        if len(tel_no.get()) > 10 or len(tel_no.get()) < 9:
            ...
            # try:    
                ...
                messagebox.showinfo("SUCCESS",otp1,parent=window)
...
def register():
    ...
    if h_name.get()=="" or reg_no.get()=="" or addr.get()=="" or tel_no.get()=="" or username.get()=="" or password.get()=="":
        ...
        try:
            ...
            mycursor=db.cursor()
            mycursor.execute("insert into resister_user (username,password,hospital_name,reg_no,address,mobile_no) values (%s,%s,%s,%s,%s,%s)",
                               (
